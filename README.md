...
[CxQueryOverride](CxQueryOverride) | A python based utility that would allow the user 1) to change the severity of the query, add the query to multiple presets at the same time.
The utility supports the batch operation to update multiple queries in one go.