...
$teamPath = "/CxServer/SP/Company/Users" # Existing Team Path - Team Level
...
function getReviewerSeverityCRUD($proxyType){
    $crudArray = @()
    $types = @("ResultStatus", "ResultSeverity", "Scan", "Project")
    foreach($type in $types){
        $crud = getItemsCRUD $proxyType $type 1 $false $false $true
        $crudArray += $crud
    }
    return $crudArray
}
...
function getRoleData($proxyType, [int]$roleId){
    ...
    $roleData.ID = $roleId
    $roleData.ItemsCRUD = getReviewerSeverityCRUD $proxyType
