...
$teamPaths = @(
    "/CxServer/SP/Company/Users", # Existing Team Path - Team Level
    "/CxServer/SP2/Company2/Team1" # Existing Team Path - Team Level
    )
...
function getReviewerCRUD($proxyType){
    $crudArray = @()
    $types = @("ResultStatus", "ResultSeverity", "Scan", "Project")
    foreach($type in $types){
        $crud = getItemsCRUD $proxyType $type 1 $false $false $false
        $crudArray += $crud
    }
    return $crudArray
}
...
function getRoleData($proxyType, [int]$roleId){
    ...
    $roleData.ID = $roleId
    $roleData.ItemsCRUD = getReviewerCRUD $proxyType
