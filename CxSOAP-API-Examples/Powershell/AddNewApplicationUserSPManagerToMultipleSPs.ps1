...
$teamPaths = @(
    "/CxServer/SP", # Existing Team Path - SP Level
    "/CxServer/SP2"  # Existing Team Path - SP Level
)
######## Get Proxy ########
...
function getSPManagerCRUD($proxyType){
    $crudArray = @()
    $types = @("ResultStatus", "ResultSeverity", "Scan", "Project")
    foreach($type in $types){
        $crud = getItemsCRUD $proxyType $type 4 $false $false $false
        $crudArray += $crud
    }
    return $crudArray
}
...
function getRoleData($proxyType, [int]$roleId){
    ...
    $roleData.ID = $roleId
    $roleData.ItemsCRUD = getSPManagerCRUD $proxyType
