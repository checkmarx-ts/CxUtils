...
$firstName = "Company"
$lastName = "Manager"
$email = "company.manager@cx"
$password = "Admin123."
$isAuditor = $false
$isActive = $true
$jobTitle = "Company Manager"
$cellPhone = "999999999"
$phone = "999999999"
$allowedIps = @()
$limitIpAccess = $false
...
$teamPaths = @(
    "/CxServer/SP/Company", # Existing Team Path - Company Level
    "/CxServer/SP2/Company2"  # Existing Team Path - Company Level
    )
...
function getCompanyManagerCRUD($proxyType){
    $crudArray = @()
    $types = @("ResultStatus", "ResultSeverity", "Scan", "Project")
    foreach($type in $types){
        $crud = getItemsCRUD $proxyType $type 2 $false $false $false
        $crudArray += $crud
    }
    return $crudArray
}
...
function getRoleData($proxyType, [int]$roleId){
    ...
    $roleData.ID = $roleId
    $roleData.ItemsCRUD = getCompanyManagerCRUD $proxyType
